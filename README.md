...
公司内部产品视频工作的独立 Codex 插件。它只读访问公司 NAS 产品资料、共享素材和批准模板脚本，使用本机 SQLite 保存任务、审批、成本与恢复记录，使用本机工作目录保存临时运行数据，并通过官方 ChatCut 插件执行可编辑剪辑、批量混剪、生成、质检和导出。
...
- 从独立只读模板根按产品、平台、画幅、语言、时长和标签解析已批准模板。
- 为最多 100 个输出生成命名 ChatCut 版本、逐镜头素材映射和可复现选择清单哈希。
