...
6. 把脚本拆为逐镜头硬约束，生成包含素材哈希、时间位置和匹配理由的选择清单；未解决镜头阻止 ChatCut 导入。
7. 在 ChatCut 中建立或绑定项目，仅导入选择清单中的最小素材集合，按清单位置剪辑并保留旧时间线。
8. 付费生成、破坏性修改和导出分别取得当前用户对本次精确范围的确认，并写入审批记录。
9. 把选择清单摘要、项目、时间线、生成、导出和成本引用写回 SQLite。
10. 质检通过后按确认导出；失败时从最后一个有证据的事件恢复。
...
- 预检拒绝把 SQLite 或工作目录配置到 UNC、映射网络盘以及产品/素材根内。
