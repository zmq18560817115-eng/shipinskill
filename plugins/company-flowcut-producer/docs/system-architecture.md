...
9. 付费生成、破坏性修改和导出分别取得当前用户对本次精确范围的确认，并写入审批记录。
10. 把模板/请求/素材目录/选择清单摘要、项目、时间线、生成、导出和成本引用写回 SQLite。
11. 质检通过后按确认导出；失败时从最后一个有证据的事件恢复。
...
| SQLite | 任务、状态、事件、审批、成本、外部引用 | 本机读写；禁止 NAS 和源目录重叠 |
